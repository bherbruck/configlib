...
func ExampleParse_multipleMissingFields() {
	...
	type Config struct {
		APIKey      string `env:"API_KEY" cli:"api-key" required:"true"`
		DatabaseURL string `env:"DATABASE_URL" cli:"database-url" required:"true"`
		SecretKey   string `env:"SECRET_KEY" cli:"secret-key" required:"true"`
		Port        int    `env:"PORT" cli:"port" default:"8080"`
	}
	...
	//   - APIKey (env: API_KEY, cli: --api-key)
	//   - DatabaseURL (env: DATABASE_URL, cli: --database-url)
	//   - SecretKey (env: SECRET_KEY, cli: --secret-key)
}
